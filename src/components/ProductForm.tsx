...
import { ArrowLeft, Camera, Lock, ChevronDown, ChevronUp, X, Check, Loader2 } from "lucide-react";
...
export default function ProductForm({ product, initialBarcode, catalogData, onSave, onCancel }: Props) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[60] bg-[#0f0f0f] flex flex-col">
        <header className="sticky top-0 z-10 bg-[#1a1a1a] border-b border-white/[0.08] px-4 py-3 flex items-center gap-3">
          <button
            type="button"
            onClick={onCancel}
            aria-label="Volver"
            className="w-9 h-9 rounded-full bg-[#242424] border border-white/[0.08] flex items-center justify-center text-[#f0f0f0]"
          >
            <ArrowLeft size={18} />
          </button>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-[#1a1a1a] border-t border-white/[0.08] px-4 pt-3 flex items-center gap-2 flex-shrink-0" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom))" }}>
              <button
                type="button"
                onClick={onCancel}
                className="flex-1 h-12 rounded-[10px] bg-[#242424] border border-white/[0.08] text-[#999] text-sm font-medium flex items-center justify-center gap-1.5"
              >
                <X size={16} color="#666" />
                Cancelar
              </button>
              <button
                type="submit"
                disabled={loading}
                className="flex-1 h-12 rounded-[10px] bg-[#00e5a0] text-black text-[15px] font-bold flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed transition-opacity duration-150"
              >
                {loading ? (
                  <>
                    <Loader2 size={16} color="#000" className="animate-spin" />
          ...
          {error && (
            <div
              className="fixed left-4 right-4 z-[52] px-4 py-2.5 rounded-[10px] bg-[#ff6b6b]/[0.10] border border-[#ff6b6b]/20 text-[13px] text-[#ff6b6b] text-center"
